    ...
    const gameLogic = () => {
      ...
      const world = (parameters, levels) => {
                ...
                if (!tileDefinitions[id].memoryLoop) {
                  if (tileDefinitions[id].canAnimate) {
                    ...
                    tileDefinitions[id].frame +=
                      tileDefinitions[id].animationSpeed;
                  }
                  // 프레임이 특정 값을 넘으면 초기화
                  if (
                    tileDefinitions[id].frame >=
        ...
        const leftSpikes = () => {
          ...
          if (spawnBonus || util.random(0, 100) < 66) {
            // 코인 습득 이후는 100%, 아닌 경우에는 66% 확률로 코인 생성
            spawnBonus = false;
        ...
        const phase = (phase) => {
          ...
          switch (phase) {
            case "menu":
              if (score > 0) {
                addGameData("1534c734f1b580f2894df5cf7e8b8358", score);
